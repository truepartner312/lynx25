...
bool PeerManagerImpl::TipMayBeStale()
{
    ...
    return m_last_tip_update.load() < GetTime<std::chrono::seconds>() - std::chrono::seconds{consensusParams.GetPowTargetSpacing(m_chainman.ActiveChain().Height()) * 3} && mapBlocksInFlight.empty();
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            ...
            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / m_chainparams.GetConsensus().GetPowTargetSpacing(pindex->nHeight);
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    ...
                    peer->m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                        (
                         ...
                         Ticks<std::chrono::seconds>(GetAdjustedTime() - m_chainman.m_best_header->Time()) / consensusParams.GetPowTargetSpacing(m_chainman.ActiveChain().Height())
                        );
                    nSyncStarted++;
                }
            }
